...
function getLinha(body, nome) {
	...
	var linha = {
		numero: unescape(encodeURIComponent($('#miolointerna u').text())).clearText().toUpperCase().replace("CONSULTA DE ITINERÏ¿½RIOS - LINHA ", ""),
		nome: nome,
		coordenador : {
	...
	common.httpGet(url).then(function (res) {
		return common.loadBody(res).then(function (body) {
			...
			for(i = 0; i < linhas.length; i += 1) {
				...
				var nome = unescape(encodeURIComponent($(linhas[i]).text())).replace("ï¿½", "Ã").replace("Ãï¿½", "ÇÃ");
				url_linhas.push({promise: common.httpGet(url_linha).then(common.loadBody), nome: nome });
			}
			return url_linhas;
		});
